...
#define DEBUG_BREAK debugtrap()
		...
		xnAudioSource* xnAudioSourceCreate(xnAudioListener* listener, int sampleRate, int maxNBuffers, npBool mono, npBool spatialized, npBool streamed, npBool hrtf, float directionFactor, int environment, float distanceScale)
		{
			(void)spatialized;
			(void)distanceScale;
